'''
Load full text (FDSys sourced) data files on disk into PostgreSQL table.

For TXT files. XML files is a separate script.
    ...
    with engine.connect() as conn:
        ...
        for indx, (root, dirs, files) in enumerate(os.walk(os.path.expanduser(data_dir))):
            for f in files:
                ...
                if fname.endswith("document.txt"):
